...
class WorkspaceMigrator:
    ...
    def analyze_local_workspace(self, workspace_path: Path) -> Dict[str, any]:
        ...
        if pipelines_dir.exists():
            ...
            pipeline_files = list(pipelines_dir.glob("*.yaml")) + list(
        ...
        if articles_dir.exists():
            ...
            article_files = list(articles_dir.glob("*.md")) + list(
        ...
        analysis["recommended_workspace_name"] = self._suggest_workspace_name(
            workspace_path
        )

        return analysis

    def migrate_local_workspace(
        self,
        workspace_path: Path,
        ...
        if self.workspace_manager.workspace_exists(target_workspace_name):
            if not overwrite:
                return (
                    False,
                    f"Workspace '{target_workspace_name}' already exists. Use overwrite=True to replace it.",
                )
...
def find_and_migrate_workspaces(
    workspace_manager,
