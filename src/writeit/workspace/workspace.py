...
class WorkspaceConfig(BaseModel):
    ...
    default_pipeline: Optional[str] = Field(
        None, description="Default pipeline template"
    )
    llm_providers: Dict[str, str] = Field(
...
class GlobalConfig(BaseModel):
    ...
    active_workspace: str = Field(
        default="default", description="Currently active workspace"
    )
    workspaces: List[str] = Field(
        default_factory=list, description="List of available workspaces"
    )
    writeit_version: str = Field(
        default="0.1.0", description="WriteIt version that created this config"
    )
...
class Workspace:
    ...
    def remove_workspace(self, name: str) -> None:
        ...
        if name == self.get_active_workspace():
            raise ValueError(
                "Cannot remove active workspace. Switch to another workspace first."
            )
