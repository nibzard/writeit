...
class TemplateLocation:
    ...
    def __init__(
        self, path: Path, scope: TemplateScope, workspace_name: Optional[str] = None
    ):
        self.path = path
        self.scope = scope
...
class TemplateManager:
    ...
    def create_template(
        ...
        if scope == TemplateScope.GLOBAL:
            ...
            raise ValueError(
                f"Cannot create template with scope '{scope}'. Use 'workspace' or 'global'."
            )
    ...
    def copy_template(
        ...
        source_location = self.resolve_template(
            source_name, template_type, source_workspace, TemplateScope.AUTO
        )
        ...
        return self.create_template(
            dest_name, template_type, content, dest_workspace, dest_scope
        )
    ...
    def _get_search_locations(
        ...
        if scope in (TemplateScope.AUTO, TemplateScope.WORKSPACE):
            ...
            locations.append(
                TemplateLocation(
                    workspace_dir / filename, TemplateScope.WORKSPACE, workspace_name
                )
            )
        ...
        if scope in (TemplateScope.AUTO, TemplateScope.GLOBAL):
            ...
            locations.append(
                TemplateLocation(global_dir / filename, TemplateScope.GLOBAL)
            )

        return locations

    def _get_search_directories(
        self, template_type: TemplateType, workspace_name: str, scope: TemplateScope
    ...
    def _get_workspace_directory(
        self, template_type: TemplateType, workspace_name: str
    ) -> Path:
