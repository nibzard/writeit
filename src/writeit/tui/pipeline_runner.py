...
class PipelineInputsWidget(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        for input_config in self.inputs:
            yield Static(f"**{input_config.label}**{'*' if input_config.required else ''}")
...
class StepExecutionWidget(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(classes="action-buttons"):
            ...
            yield Button("Continue", id="continue-step", variant="success", disabled=True)
...
class PipelineSummaryWidget(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static("Your pipeline has completed successfully. Review the final output below:")
...
class PipelineRunnerApp(App[None]):
    ...
    BINDINGS = [
        ...
        Binding("shift+tab", "focus_previous_enhanced", "Previous Field (Enhanced)", show=False),
    ]
    ...
    def action_copy_response(self) -> None:
        ...
        try:
            ...
            self.update_status_bar("❌ pyperclip not installed - can't copy to clipboard")
    ...
    async def execute_current_step(self) -> None:
        ...
        if not self.pipeline_config or self.current_step_index >= len(self.pipeline_config.steps):
    ...
    async def execute_step_with_llm(self, step: PipelineStep, widget: StepExecutionWidget) -> None:
        ...
        try:
            ...
            model_name = step.model_preference[0] if isinstance(step.model_preference, list) else step.model_preference
            if not model_name:
            ...
            for key, value in self.pipeline_config.defaults.items():
                if isinstance(value, dict):
                    for sub_key, sub_value in value.items():
                        model_name = model_name.replace(f"{{{{ defaults.{key}.{sub_key} }}}}", str(sub_value))
                else:
            ...
            async def update_thinking_status():
                ...
                while True:
                    ...
                    dots = "." * ((len(dots) + 1) % 4)  # Cycle through "", ".", "..", "..."
                    self.update_status_bar(f"🤖 {model_name} is thinking{dots} (10-30 seconds)")
            ...
            if self.current_run_id:
                ...
                if step_usage and self.token_tracker.current_run:
                    ...
                    total_tokens = current_run.total_input_tokens + current_run.total_output_tokens
    ...
    def render_prompt_template(self, step: PipelineStep) -> str:
        ...
        if hasattr(self, 'step_responses'):
            for step_key, response in self.step_responses.items():
                template = template.replace(f"{{{{ steps.{step_key}.selected_response }}}}", response)
