...
class WriteItClient:
    ...
    async def create_pipeline(
        ...
        async with self.session.post(
            f"{self.config.base_url}/api/pipelines",
            json=data
        ) as response:
    ...
    async def execute_run(
        ...
        async with self.session.post(
            f"{self.config.base_url}/api/runs/{run_id}/execute",
            params=params
        ) as response:
            response.raise_for_status()
            return await response.json()
    ...
    async def get_run(
        ...
        async with self.session.get(
            f"{self.config.base_url}/api/runs/{run_id}",
            params=params
        ) as response:
            response.raise_for_status()
            return await response.json()
...
class PipelineClient:
    ...
    async def get_pipeline_info(self, pipeline_path: Path) -> Dict[str, Any]:
        ...
        async with self.client:
            ...
            pipeline_resp = await self.client.create_pipeline(pipeline_path, self.workspace_name)
