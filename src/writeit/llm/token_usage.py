...
class TokenUsage:
    ...
    def cost_estimate(self) -> Optional[Decimal]:
        ...
        input_cost = (Decimal(self.input_tokens) / Decimal("1000000")) * pricing[
            "input"
        ]
        output_cost = (Decimal(self.output_tokens) / Decimal("1000000")) * pricing[
            "output"
        ]
...
class PipelineRunTokens:
    ...
    def add_step_usage(
        self, step_key: str, step_name: str, model_name: str, response
    ) -> StepTokenUsage:
        """Add token usage for a pipeline step."""
        usage = TokenUsage.from_llm_response(response)
        step_usage = StepTokenUsage(
            step_key=step_key, step_name=step_name, model_name=model_name, usage=usage
        )
...
class TokenUsageTracker:
    ...
    def start_pipeline_run(self, pipeline_name: str, run_id: str) -> PipelineRunTokens:
        ...
        self.current_run = PipelineRunTokens(
            pipeline_name=pipeline_name, run_id=run_id, start_time=datetime.now()
        )
        return self.current_run

    def track_step_usage(
        self, step_key: str, step_name: str, model_name: str, response
    ) -> Optional[StepTokenUsage]:
        """Track token usage for a pipeline step."""
        if not self.current_run:
            return None

        return self.current_run.add_step_usage(
            step_key, step_name, model_name, response
        )
