...
class DocumentationGenerator:
    ...
    def _generate_api_docs(self) -> Optional[APIDocumentation]:
        ...
        try:
            ...
            if Path(spec_path).exists():
                return self.extractors.api_extractor.extract_from_openapi_file(
                    Path(spec_path)
                )
    ...
    def _generate_module_docs(self) -> List[ModuleDocumentation]:
        ...
        for pattern in patterns:
            for py_file in base_path.rglob(pattern):
                ...
                try:
                    module_doc = self.extractors.module_extractor.extract_module(
                        py_file
                    )
                    if module_doc:
    ...
    def _update_module_metrics(self, module_doc: ModuleDocumentation):
        ...
        documented_classes = sum(
            1 for cls in module_doc.classes if cls.description and cls.purpose
        )
        self.metrics.documented_classes += documented_classes

        self.metrics.total_functions += len(module_doc.functions)
        documented_functions = sum(
            1 for func in module_doc.functions if func.description
        )
    ...
    def validate_generation(self) -> bool:
        ...
        if metrics.overall_coverage < 50:
            print(
                f"Warning: Low documentation coverage ({metrics.overall_coverage:.1f}%)"
            )

        if metrics.generation_time > 60:
            print(
                f"Warning: Documentation generation took {metrics.generation_time:.1f} seconds"
            )
