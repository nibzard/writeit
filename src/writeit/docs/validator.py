...
from .models import DocumentationSet, ValidationResult, DocumentationMetrics
...
class DocumentationValidator:
    ...
    def _validate_api_docs(self, api_docs, results: ValidationResult):
        ...
        if not api_docs.endpoints:
            results.add_error(
                ...
                suggestion="Check FastAPI app configuration",
            )
            return

        # Update metrics
        self.metrics.total_api_endpoints = len(api_docs.endpoints)
        self.metrics.documented_api_endpoints = len(
            [e for e in api_docs.endpoints if e.description]
        )
    ...
    def _validate_endpoint(self, endpoint, results: ValidationResult):
        ...
        if not endpoint.path:
            results.add_error(
                "endpoint_missing_path", f"Endpoint missing path: {endpoint.method}"
            )

        if not endpoint.method:
            results.add_error(
                "endpoint_missing_method", f"Endpoint missing method: {endpoint.path}"
            )

        if not endpoint.description:
            results.add_warning(
                "endpoint_missing_description",
                f"Endpoint missing description: {endpoint.method} {endpoint.path}",
            )

        # Validate path format
        if endpoint.path and not endpoint.path.startswith("/"):
            results.add_error(
                "endpoint_invalid_path", f"Invalid path format: {endpoint.path}"
            )
        ...
        if endpoint.method not in valid_methods:
            results.add_error(
                "endpoint_invalid_method", f"Invalid HTTP method: {endpoint.method}"
            )
    ...
    def _validate_module(self, module, results: ValidationResult):
        ...
        if not module.description:
            results.add_warning(
                ...
                f"Module missing description: {module.name}",
            )

        if not module.purpose:
            results.add_warning(
                "module_missing_purpose", f"Module missing purpose: {module.name}"
            )
    ...
    def _validate_class(self, class_doc, results: ValidationResult):
        ...
        if not class_doc.description:
            results.add_warning(
                "class_missing_description",
                f"Class missing description: {class_doc.name}",
            )
    ...
    def _validate_function(self, func_doc, results: ValidationResult):
        ...
        if not func_doc.description:
            results.add_warning(
                "function_missing_description",
                f"Function missing description: {func_doc.name}",
            )

        if not func_doc.signature:
            results.add_error(
                "function_missing_signature",
                f"Function missing signature: {func_doc.name}",
            )
        ...
        for param in func_doc.parameters:
            if not param.name:
                results.add_error(
                    ...
                    f"Parameter missing name in {func_doc.name}",
                )

            if not param.type_annotation:
                results.add_warning(
                    "parameter_missing_type",
                    f"Parameter missing type annotation: {func_doc.name}.{param.name}",
                )
    ...
    def _validate_command(self, command, results: ValidationResult):
        ...
        if not command.description:
            results.add_warning(
                ...
                f"Command missing description: {command.name}",
            )

        if not command.usage:
            results.add_warning(
                "command_missing_usage", f"Command missing usage: {command.name}"
            )

    def _validate_code_examples(
        self, docs: DocumentationSet, results: ValidationResult
    ):
        """Validate code examples"""
        all_examples = []
    ...
    def _validate_example(self, example, results: ValidationResult) -> bool:
        ...
        if not example.code:
            results.add_error(
                "example_missing_code", f"Example missing code: {example.description}"
            )
            return False

        if not example.description:
            results.add_warning(
                "example_missing_description", "Example missing description"
            )
    ...
    def _validate_python_example(self, example, results: ValidationResult) -> bool:
        ...
        try:
            ...
            if all(
                ...
                results.add_warning(
                    "example_incomplete",
                    "Example appears to be incomplete doctest format",
                )
                return False
            ...
            for pattern in placeholder_patterns:
                if pattern.lower() in example.code.lower():
                    results.add_warning(
                        "example_has_placeholder",
                        f"Example contains placeholder: {pattern}",
                    )
                    break
            ...
            results.add_error(
                "example_syntax_error", f"Python syntax error in example: {e}"
            )
            return False
        except Exception as e:
            results.add_error(
                "example_validation_error", f"Error validating Python example: {e}"
            )
            return False
    ...
    def _validate_bash_example(self, example, results: ValidationResult) -> bool:
        ...
        for cmd in dangerous_commands:
            if cmd in code:
                results.add_error(
                    "example_dangerous_command",
                    f"Example contains dangerous command: {cmd}",
                )
                return False
        ...
        if code.count('"') % 2 != 0 or code.count("'") % 2 != 0:
            results.add_warning(
                "example_unmatched_quotes", "Example may have unmatched quotes"
            )
    ...
    def _validate_links(self, docs: DocumentationSet, results: ValidationResult):
        ...
        if docs.module_docs:
            for module in docs.module_docs:
                ...
                for class_doc in module.classes:
                    ...
                    internal_refs.update(
                        self._extract_internal_refs(class_doc.description)
                    )
                    for method in class_doc.methods:
                        all_links.update(self._extract_links(method.description))
                        internal_refs.update(
                            self._extract_internal_refs(method.description)
                        )
                for func_doc in module.functions:
                    all_links.update(self._extract_links(func_doc.description))
                    internal_refs.update(
                        self._extract_internal_refs(func_doc.description)
                    )
        ...
        for link in all_links:
            if self._is_external_link(link) and self._is_broken_link(link):
                results.add_error(
                    "broken_external_link", f"Broken external link: {link}"
                )
        ...
        for ref in internal_refs:
            if not self._validate_internal_reference(ref, docs):
                results.add_error(
                    "broken_internal_ref", f"Broken internal reference: {ref}"
                )
        ...
        if all_links:
            results.add_info(
                "link_summary",
                f"Validated {len(all_links)} external links and {len(internal_refs)} internal references",
            )
    ...
    def _validate_completeness(self, docs: DocumentationSet, results: ValidationResult):
        ...
        if docs.api_docs:
            ...
            documented_items += len(
                [e for e in docs.api_docs.endpoints if e.description]
            )
        ...
        if total_items > 0:
            ...
            if coverage < 50:
                results.add_warning(
                    "low_coverage", f"Low documentation coverage: {coverage:.1f}%"
                )
            elif coverage < 80:
                results.add_info(
                    "moderate_coverage",
                    f"Moderate documentation coverage: {coverage:.1f}%",
                )
            else:
                results.add_info(
                    "high_coverage", f"High documentation coverage: {coverage:.1f}%"
                )
    ...
    def validate_api_consistency(self) -> ValidationResult:
        ...
        try:
            ...
            results.add_info(
                "api_consistency_checked", "API consistency check completed"
            )

        except ImportError:
            results.add_warning(
                "api_app_not_found", "FastAPI app not found for consistency check"
            )
        except Exception as e:
            results.add_error(
                "api_consistency_error", f"Error checking API consistency: {e}"
            )
