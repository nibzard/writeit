...
class HTMLDocumentationGenerator:
    ...
    def deploy_html(
        self, docs: DocumentationSet, output_path: Path, markdown_path: Path
    ):
    ...
    def _create_mkdocs_config(self, docs: DocumentationSet) -> Dict[str, Any]:
        ...
        config = {
            "site_name": docs.api_docs.title
            if docs.api_docs
            else "WriteIt Documentation",
...
"""
    ...
    def _build_with_mkdocs(self, output_path: Path):
        ...
        try:
            ...
            print(
                f"⚠️  MkDocs not available, markdown files deployed to {output_path}/docs"
            )
