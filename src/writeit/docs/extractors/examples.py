...
class ExampleExtractor:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                    function_examples = self._extract_from_test_function(node, test_file)
    ...
    def _extract_from_test_function(self, node: ast.FunctionDef, test_file: Path) -> List[CodeExample]:
    ...
    def _clean_test_example(self, code: str) -> str:
        ...
        for line in lines:
            ...
            if line.strip().startswith('assert '):
                ...
                if ' == ' in line and not any(skip in line for skip in ['None', 'True', 'False']):
    ...
    def validate_examples(self, examples: List[CodeExample]) -> List[CodeExample]:
        ...
        for example in examples:
            if self._validate_example_syntax(example):
                ...
                print(f"Warning: Invalid example syntax in: {example.description[:50]}...")
    ...
    def extract_from_directory(self, directory: Path, file_pattern: str = "*.py") -> List[CodeExample]:
