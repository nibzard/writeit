...
from ..models import CLIDocumentation, CommandDocumentation, ParameterDocumentation
...
class CLIExtractor:
    ...
    def _extract_command_parameters(
        self, sig, cmd_info: CommandInfo
    ) -> List[ParameterDocumentation]:
    ...
    def _extract_command_examples(self, cmd_info: CommandInfo) -> List[str]:
        ...
        if cmd_info.callback:
            ...
            optional_params = [
                ...
                if name not in ["ctx", "typer_context"]
    ...
    def _extract_command_tags(self, cmd_info: CommandInfo) -> List[str]:
        ...
        if any(
            keyword in cmd_info.name.lower() for keyword in ["init", "setup", "config"]
        ):
            tags.append("setup")
        elif any(
            keyword in cmd_info.name.lower() for keyword in ["run", "execute", "start"]
        ):
            tags.append("execution")
        elif any(
            keyword in cmd_info.name.lower() for keyword in ["list", "show", "get"]
        ):
            tags.append("query")
        elif any(
            keyword in cmd_info.name.lower() for keyword in ["create", "add", "new"]
        ):
            tags.append("creation")
        elif any(
            keyword in cmd_info.name.lower() for keyword in ["delete", "remove", "rm"]
        ):
            tags.append("deletion")
        elif any(
            keyword in cmd_info.name.lower()
            for keyword in ["validate", "check", "test"]
        ):
