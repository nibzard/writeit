...
class PDFDocumentationGenerator:
    ...
    def _generate_from_html(
        self, docs: DocumentationSet, output_path: Path, html_path: Path
    ) -> bool:
    ...
    def _generate_from_content(self, docs: DocumentationSet, output_path: Path) -> bool:
        ...
        try:
            ...
            story.append(
                Paragraph(
                    f"Generated on {docs.generated_at.strftime('%B %d, %Y')}",
                    styles["Normal"],
                )
            )
            ...
            if docs.api_docs:
                toc_data.append(
                    ["API Documentation", f"{len(docs.api_docs.endpoints)} endpoints"]
                )
            if docs.module_docs:
                toc_data.append(
                    ["Module Documentation", f"{len(docs.module_docs)} modules"]
                )
            if docs.cli_docs:
                toc_data.append(
                    ["CLI Documentation", f"{len(docs.cli_docs.commands)} commands"]
                )
            if docs.user_guides:
            ...
            if docs.api_docs:
                ...
                for endpoint in docs.api_docs.endpoints[:10]:
                    story.append(
                        Paragraph(
                            f"{endpoint.method} {endpoint.path}", styles["Heading4"]
                        )
                    )
                    ...
                    if endpoint.parameters:
                        param_text = "Parameters: " + ", ".join(
                            [p.name for p in endpoint.parameters[:5]]
                        )
            ...
            if docs.module_docs:
                ...
                for module in docs.module_docs[:5]:
                    story.append(
                        Paragraph(f"Module: {module.name}", styles["Heading3"])
                    )
                    story.append(Paragraph(module.description, styles["Normal"]))
                    story.append(
                        Paragraph(f"Purpose: {module.purpose}", styles["Normal"])
                    )

                    if module.classes:
                        story.append(
                            Paragraph(
                                f"Classes ({len(module.classes)})", styles["Heading4"]
                            )
                        )
            ...
            if docs.cli_docs:
                ...
                for command in docs.cli_docs.commands[:10]:
                    story.append(
                        Paragraph(f"Command: {command.name}", styles["Heading4"])
                    )
            ...
            if docs.user_guides:
                ...
                for guide in docs.user_guides:
                    ...
                    story.append(
                        Paragraph(
                            f"Audience: {guide.audience} | Difficulty: {guide.difficulty} | Time: {guide.estimated_time}",
                            styles["Normal"],
                        )
                    )
            ...
            story.append(
                Paragraph(
                    f"This documentation was automatically generated on {docs.generated_at.strftime('%Y-%m-%d %H:%M:%S')}.",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(f"WriteIt Version: {docs.version}", styles["Normal"])
            )
            story.append(
                Paragraph(
                    "This is an auto-generated document. Please report any issues.",
                    styles["Italic"],
                )
            )
