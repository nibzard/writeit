...
class DocumentationDeployment:
    ...
    def deploy(self, docs: DocumentationSet, output_path: Path, formats: List[str]):
        ...
        for format_type in formats:
            if format_type == "markdown":
                ...
                self._deploy_pdf_enhanced(
                    docs, output_path / "writeit-documentation.pdf", output_path
                )
            else:
                print(f"Unknown format: {format_type}")

    def _deploy_markdown_with_templates(
        self, docs: DocumentationSet, output_path: Path
    ):
    ...
    def _deploy_with_template_system(self, docs: DocumentationSet, output_path: Path):
        ...
        if docs.api_docs:
            try:
                api_content = self.template_system.render_api_documentation(
                    docs.api_docs
                )
        ...
        if docs.module_docs:
            try:
                modules_content = self.template_system.render_module_documentation(
                    docs.module_docs
                )
        ...
        if docs.cli_docs:
            try:
                cli_content = self.template_system.render_cli_documentation(
                    docs.cli_docs
                )
        ...
        if docs.user_guides:
            ...
            for guide in docs.user_guides:
                guides_index += (
                    f"- [{guide.title}]({guide.title.replace(' ', '-').lower()}.md)\n"
                )
...
"""
        ...
        for endpoint in api_docs.endpoints:
            ...
            overview_content += (
                f"- [{endpoint.method} {endpoint.path}](endpoints/{filename})\n"
            )
...
"""
    ...
    def _write_module_markdown(self, module_docs: List, output_path: Path):
        ...
        for module in module_docs:
            overview_content += (
                f"- [{module.name}]({module.name.replace('.', '/')}.md)\n"
            )
            ...
            for template in template_docs.templates:
                ...
                if template.steps:
                    ...
                    for step in template.steps:
                        content += (
                            f"- **{step.name}** ({step.type}): {step.description}\n"
                        )
                    content += "\n"
...
"""
    ...
    def _deploy_html_fallback(self, docs: DocumentationSet, output_path: Path):
        ...
        try:
            ...
            index_file = (
                ...
                else output_path / "index.md"
            )
            readme_file = (
                docs_dir / "README.md"
                if "docs_dir" in locals()
                else output_path / "README.md"
            )
    ...
    def _deploy_pdf_enhanced(
        self, docs: DocumentationSet, output_path: Path, base_output_path: Path
    ):
        ...
        try:
            ...
            success = pdf_generator.generate_pdf(
                docs, output_path, html_path if html_path.exists() else None
            )
...
"""
    ...
    def deploy_to_github_pages(self, site_path: Path):
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["gh-pages", "--dist", str(site_path), "--dotfiles"],
                    check=True,
                    capture_output=True,
                )
    ...
    def serve_local_preview(
        self, docs_path: Path, host: str = "127.0.0.1", port: int = 8000
    ):
