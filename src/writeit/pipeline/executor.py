...
from writeit.models import Pipeline, PipelineStep as ModelStep, PipelineRun, StepExecution
...
class PipelineExecutor:
    ...
    def __init__(self, workspace: Workspace, storage: StorageManager, workspace_name: str = "default"):
    ...
    async def load_pipeline(self, pipeline_path: Path) -> Pipeline:
        ...
        try:
            ...
            raise PipelineError(f"Failed to load pipeline from {pipeline_path}: {str(e)}")
    ...
    async def execute_run(
        ...
        try:
            ...
            for i, step in enumerate(pipeline.steps):
                ...
                context.step_outputs[step.key] = step_result.selected_response or step_result.responses[0]
    ...
    async def execute_step(
        ...
        try:
            ...
            return StepResult(
                ...
                tokens_used=context.token_tracker.get_step_usage(step.key) if context.token_tracker else {}
            )
    ...
    def _select_model(self, preferences: List[str], defaults: Dict[str, Any]) -> str:
        ...
        model_name = preferences[0] if isinstance(preferences, list) else str(preferences)
    ...
    async def _execute_llm_call(
        ...
        try:
            ...
            raise StepExecutionError(f"LLM call failed for model {model_name}: {str(e)}")
    
    async def _validate_inputs(self, pipeline: Pipeline, inputs: Dict[str, Any]) -> None:
            ...
            if key in inputs:
                ...
                if expected_type == 'text' and not isinstance(value, str):
                    ...
                    if value not in valid_options:
                        raise ValidationError(f"Input '{key}' must be one of: {valid_options}")
    ...
    async def get_run(self, run_id: str) -> Optional[PipelineRun]:
        ...
        try:
            run_data = await self.storage.get_json(f"pipeline_run_{run_id}", db_name="pipeline_runs")
            if run_data:
