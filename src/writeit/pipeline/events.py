...
class PipelineEventStore:
    ...
    async def get_events(self, run_id: str, from_sequence: int = 0) -> List[PipelineEvent]:
    ...
    async def get_state_at_version(self, run_id: str, version: int) -> Optional[PipelineState]:
    ...
    async def _apply_event(self, state: PipelineState, event: PipelineEvent) -> PipelineState:
    ...
    def _update_step_in_run(self, run: PipelineRun, updated_step: StepExecution) -> List[StepExecution]:
    ...
    async def _store_event(self, event: PipelineEvent) -> None:
        ...
        await self.storage.store_json(key, event.to_dict(), db_name="pipeline_events")
