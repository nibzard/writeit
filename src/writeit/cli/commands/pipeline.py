...
app = typer.Typer(name="pipeline", help="Pipeline operations", rich_markup_mode="rich")
...
def list_pipelines():
    ...
    try:
        ...
        if not templates:
            ...
            console.print(
                "  • [primary]writeit template create <name>[/primary] (workspace scope)"
            )
            console.print(
                "  • [primary]writeit template create <name> --global[/primary] (global scope)"
            )
            return
...
def run(
    ...
    use_global: bool = typer.Option(
        False, "--global", "-g", help="Use global pipeline template only"
    ),
    ...
    try:
        ...
        console.print(
            f"[primary]Pipeline:[/primary] [pipeline.name]{pipeline}[/pipeline.name]"
        )
        console.print(f"[primary]Path:[/primary] [path]{pipeline_path}[/path]")
        scope_label = (
            "Global"
            if location.scope == TemplateScope.GLOBAL
            else f"Workspace ({location.workspace_name})"
        )
        console.print(f"[primary]Scope:[/primary] {scope_label}")
        console.print(
            f"[primary]Workspace:[/primary] [workspace.active]{workspace_name}[/workspace.active]"
        )
        console.print()
