...
from writeit.cli.output import (
    ...
    create_workspace_table, create_workspace_info_table,
...
app = typer.Typer(
    name="workspace", 
    help="Manage WriteIt workspaces",
...
def create(
    ...
    set_active: bool = typer.Option(
        ...
        "--set-active",
        "-s",
        help="Set as active workspace after creation"
    )
):
    """
...
def list_workspaces():
    ...
    try:
        ...
        if not workspaces:
            ...
            console.print("Create a workspace with: [primary]writeit workspace create <name>[/primary]")
            return
...
def use(
    name: str = typer.Argument(
        ...
        help="Name of the workspace to use"
    )
):
    """
...
def remove(
    ...
    try:
        ...
        if not force:
            ...
            console.print(f"[warning]You are about to remove workspace '[bold]{name}[/bold]'[/warning]")
            console.print(f"[secondary]Path: {workspace_path}[/secondary]")
            console.print("[error]This will permanently delete all data in this workspace.[/error]")
            
            if not confirm_with_style("Are you sure you want to continue?", default=False):
