...
def resolve_template_path(
    ...
    if detected_type:
        template_type = (
            TemplateType.PIPELINE if detected_type == "pipeline" else TemplateType.STYLE
        )
        location = template_manager.resolve_template(
            filename, template_type, workspace_name, scope
        )
...
def detect_file_type(file_path: Path) -> Optional[str]:
    ...
    try:
        ...
        if any(
            word in name
            for word in ["pipeline", "template", "article", "blog", "guide"]
        ):
            return "pipeline"
...
def validate(
    ...
    summary_only: bool = typer.Option(
        False, "--summary-only", "-s", help="Show only summary, not individual issues"
    ),
    use_global: bool = typer.Option(
        False, "--global", "-g", help="Only look in global templates/styles directory"
    ),
    ...
    show_content: bool = typer.Option(
        False, "--show-content", help="Show file content with syntax highlighting"
    ),
    ...
    try:
        ...
        for file_arg in files:
            ...
            if not file_path.is_absolute():
                if local:
                    ...
                    file_path = resolve_template_path(
                        template_manager, workspace_name, file_arg, use_global
                    )
            ...
            if detected_type == FileType.auto:
                ...
                if not detected_type:
                    print_warning(
                        f"Could not auto-detect type for {file_path}, skipping"
                    )
                    continue

            # Show what we're validating
            console.print(
                f"[primary]Validating:[/primary] [path]{file_path}[/path] [secondary]({detected_type})[/secondary]"
            )
        ...
        if summary_only:
            ...
            format_validation_results(
                results, detailed=detailed, show_suggestions=detailed
            )
