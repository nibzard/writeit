...
def generate_docs(
    ...
    format: List[str] = typer.Option(
        ["markdown"], "--format", "-f", help="Output format(s)"
    ),
    config: Optional[Path] = typer.Option(
        None, "--config", "-c", help="Configuration file path"
    ),
    validate: bool = typer.Option(
        True, "--validate/--no-validate", help="Validate documentation after generation"
    ),
...
def validate_docs(
    ...
    detailed: bool = typer.Option(
        False, "--detailed", help="Show detailed validation results"
    ),
    fix_issues: bool = typer.Option(
        False, "--fix", help="Attempt to fix common issues automatically"
    ),
):
...
def preview_docs(
    docs_path: Path = typer.Option(
        Path("docs/site"), "--docs-path", "-d", help="Path to documentation site"
    ),
...
def show_metrics(
    ...
    try:
        ...
        table.add_row(
            "Modules",
            str(metrics.documented_modules),
            str(metrics.total_modules),
            f"{metrics.module_coverage:.1f}%",
        )
        table.add_row(
            "Classes",
            str(metrics.documented_classes),
            str(metrics.total_classes),
            f"{metrics.class_coverage:.1f}%",
        )
        table.add_row(
            "Functions",
            str(metrics.documented_functions),
            str(metrics.total_functions),
            f"{metrics.function_coverage:.1f}%",
        )
        table.add_row(
            "API Endpoints",
            str(metrics.documented_api_endpoints),
            str(metrics.total_api_endpoints),
            f"{metrics.api_coverage:.1f}%",
        )
...
def config_docs(
    init: bool = typer.Option(
        False, "--init", help="Initialize documentation configuration"
    ),
    show: bool = typer.Option(False, "--show", help="Show current configuration"),
    config_path: Path = typer.Option(
        Path("docs/config.yaml"), "--config-path", help="Configuration file path"
    ),
):
...
def _display_generation_results(docs, metrics: DocumentationMetrics, verbose: bool):
    ...
    if docs.api_docs:
        console.print(
            f"• API Documentation: {len(docs.api_docs.endpoints)} endpoints, {len(docs.api_docs.models)} models"
        )
    ...
    if docs.module_docs:
        ...
        if verbose:
            for module in docs.module_docs:
                console.print(
                    f"  - {module.name}: {len(module.classes)} classes, {len(module.functions)} functions"
                )
    ...
    if docs.template_docs:
        console.print(
            f"• Template Documentation: {len(docs.template_docs.templates)} templates"
        )
...
def _display_validation_results(results, detailed: bool = False):
    ...
    if results.is_valid:
        ...
        console.print(
            f"• ❌ Validation failed ({len(results.errors)} errors)", style="red"
        )
