...
def print_success(message: str, title: Optional[str] = None) -> None:
    ...
    if title:
        panel = Panel(
            f"[success]{message}[/success]", title=f"✓ {title}", border_style="green"
        )
        console.print(panel)
    else:
...
def print_error(message: str, title: Optional[str] = None) -> None:
    ...
    if title:
        panel = Panel(
            f"[error]{message}[/error]", title=f"❌ {title}", border_style="red"
        )
        console.print(panel)
    else:
...
def print_warning(message: str, title: Optional[str] = None) -> None:
    ...
    if title:
        panel = Panel(
            f"[warning]{message}[/warning]", title=f"⚠️ {title}", border_style="yellow"
        )
        console.print(panel)
    else:
...
def print_info(message: str, title: Optional[str] = None) -> None:
    ...
    if title:
        panel = Panel(
            f"[info]{message}[/info]", title=f"ℹ️ {title}", border_style="cyan"
        )
        console.print(panel)
    else:
...
def create_workspace_table(
    workspaces: List[str], active_workspace: Optional[str] = None
) -> Table:
    ...
    for workspace in workspaces:
        ...
        table.add_row(
            Text(workspace, style=name_style), Text(status, style=status_style)
        )

    return table


def create_workspace_info_table(
    workspace_name: str, config: Dict[str, Any], stats: Optional[Dict[str, Any]] = None
) -> Table:
...
def show_yaml_with_highlighting(
    file_path: Path, highlight_lines: Optional[List[int]] = None
) -> None:
