...
class PipelineValidator:
    ...
    def validate_file(self, file_path: Path) -> ValidationResult:
        ...
        try:
            ...
            if isinstance(steps, dict):
                ...
                llm_step_count = len(
                    [s for s in steps.values() if s.get("type") == "llm_generation"]
                )
                ...
                llm_step_count = len(
                    [
                        s
                        for s in steps
                        if isinstance(s, dict) and s.get("type") == "llm_generation"
                    ]
                )
            else:
                step_count = 0
    ...
    def _validate_top_level_structure(
        self, pipeline: Dict[str, Any], result: ValidationResult
    ) -> None:
    ...
    def _validate_metadata(
        ...
        if not metadata:
            result.add_error(
                ...
                suggestion="Add pipeline name, description, version, and author",
            )
            return
    ...
    def _validate_inputs(
        self, inputs: Dict[str, Any], result: ValidationResult
    ) -> None:
        """Validate inputs section."""
        if not inputs:
            result.add_warning(
                "No inputs defined",
                suggestion="Consider adding input parameters for flexibility",
            )
            return
    ...
    def _validate_steps(self, steps: Dict[str, Any], result: ValidationResult) -> None:
        ...
        if not steps:
            result.add_error(
                "No steps defined", suggestion="Pipeline must have at least one step"
            )
            return

        # Handle both dictionary and list formats
        if isinstance(steps, list):
            # List format: [{"name": "step1", ...}, {"name": "step2", ...}]
            ...
            result.add_error(
                "Steps must be either a list or dictionary",
                location="steps",
            )

    def _validate_step(
        self, step_name: str, step_config: Dict[str, Any], result: ValidationResult
    ) -> None:
        ...
        if "type" not in step_config:
            result.add_error(
                f"Step '{step_name}' missing required 'type' field", location=location
            )
            return
    ...
    def _add_dict_variables(
        self, data: Dict[str, Any], prefix: str, available_vars: Set[str]
    ) -> None:
    ...
    def _validate_step_dependencies(
        self, steps: Dict[str, Any], result: ValidationResult
    ) -> None:
        ...
        if isinstance(steps, dict):
            ...
            for i, (step_name, step_config) in enumerate(steps.items()):
                self._validate_step_dependency(
                    step_name, step_config, step_names, i, result
                )
        elif isinstance(steps, list):
            step_names = []
            for step_config in steps:
                if isinstance(step_config, dict):
                    step_name = step_config.get("name", f"step_{len(step_names)}")
    ...
    def _validate_step_dependency(
        ...
        """Validate individual step dependency."""
        ...
        if step_config.get("type") == "user_selection":
            ...
            if from_step:
                ...
                elif step_names.index(from_step) >= step_index:
                    result.add_error(
