...
class ValidationResult:
    ...
    def add_error(self, message: str, location: Optional[str] = None, 
        ...
        self.issues.append(ValidationIssue(
            IssueType.ERROR, message, location, line_number, suggestion
        ))
        self.is_valid = False
    ...
    def add_info(self, message: str, location: Optional[str] = None,
        ...
        """Add an info issue."""
        self.issues.append(ValidationIssue(
            IssueType.INFO, message, location, line_number, suggestion
        ))
    ...
    def format_summary(self) -> str:
        ...
        if self.issues:
            ...
            if self.error_count > 0:
                counts.append(f"{self.error_count} error{'s' if self.error_count != 1 else ''}")
            if self.warning_count > 0:
                counts.append(f"{self.warning_count} warning{'s' if self.warning_count != 1 else ''}")
