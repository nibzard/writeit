...
class TestValidationIntegration:
    ...
    def test_tech_article_pipeline_validates(self):
        ...
        assert result.is_valid, f"tech-article.yaml should be valid. Issues: {[issue.message for issue in result.issues if issue.is_error]}"
    ...
    def test_quick_article_pipeline_validates(self):
        ...
        assert result.is_valid, f"quick-article.yaml should be valid. Issues: {[issue.message for issue in result.issues if issue.is_error]}"
    ...
    def test_technical_expert_style_validates(self):
        ...
        assert result.is_valid, f"technical-expert.yaml should be valid. Issues: {[issue.message for issue in result.issues if issue.is_error]}"
    ...
    def test_conversational_blog_style_validates(self):
        ...
        if not style_path:
            pytest.skip("conversational-blog.yaml style not found in expected locations")
        ...
        assert result.is_valid, f"conversational-blog.yaml should be valid. Issues: {[issue.message for issue in result.issues if issue.is_error]}"
    ...
    def test_business_executive_style_validates(self):
        ...
        assert result.is_valid, f"business-executive.yaml should be valid. Issues: {[issue.message for issue in result.issues if issue.is_error]}"
    ...
    def test_file_type_detection(self):
        ...
        if template_path.exists():
            ...
            assert detected_type == "pipeline", f"Expected 'pipeline' but got '{detected_type}'"
        ...
        for style_path in style_paths:
            if style_path.exists():
                ...
                assert detected_type == "style", f"Expected 'style' but got '{detected_type}'"
                break
    ...
    def test_validation_system_comprehensive(self):
        ...
        style_dirs = [
            self.repo_root / "styles",
            Path.home() / ".writeit" / "styles"
        ]
        ...
        assert summary.failed_files == 0, f"All shipped templates and styles should be valid. {summary.failed_files} files failed validation."